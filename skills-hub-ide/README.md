...
1.  **Skills Manager (`skills_manager.sh`)**: 交互式主程序，负责将源码分发到各个 IDE。
2.  **Symlink Tool (`link.sh`)**: 底层核心工具，负责创建软链接，支持命令行参数调用。
...
- `skills_manager.sh`: 主程序（交互式菜单）
- `link.sh`: 核心逻辑（软链接生成器）
- `simplebak/`: 旧版本备份

## 注意事项

- **软链接机制**：本工具使用软链接 (`ln -s`) 实现同步。这意味着你只需要维护一份源码，IDE 里看到的永远是最新版本，无需反复复制粘贴。
